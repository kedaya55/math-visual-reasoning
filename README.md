...
这是供 Codex 使用的 **v0.2 试用版 skill**，包含制作规范与可复用数学组件；动画制作、预览和导出使用 HyperFrames。它不是独立的视频生成软件。
...
- 补充数位与运算、数量关系、尺规作图、空间几何及统计概率等 39 类参数示例。
- 提供按需复制工具，自动带上本地依赖，遇到不同内容的目标文件会拒绝覆盖。
- 加强学生当前已知与推理顺序检查；先设计可追踪的图形动作，再配辅助讲稿。
- 增加共享对象衔接、同尺度对照、图形注意层次及面积重排动作案例。
...
| [assets/math/](assets/math/) | 数学模块、样式、组件预览与模型测试 |
| [统一离线预览](assets/math/library-preview.html) | 下载后在浏览器中查看基础组件 |
| [v0.2 新增交互预览](assets/math/expansion-preview.html) | 39 类新增参数示例 |
| [小学、初中知识点索引](references/curriculum-coverage.md) | 课标对应、组件选择与能力边界 |
| [按需复制工具](scripts/copy-math-components.mjs) | 复制指定模块及本地依赖 |
...
v0.1 已完成本机 21 项组件模型测试，以及隔离工作目录内的数轴动画接入、运行和关键画面检查。v0.2 新增组件完成 36 项数学模型测试、39 类示例的 241 个浏览器参数状态检查；详见 [组件 QA](references/expansion-qa.md)。该验证仍使用本机现有运行环境，不等于全新机器、无上下文制作或学生学习效果已验证。
...
已补充公式结构布局、指定剪分以及圆锥、圆柱和长方体等限定模型的展开；不提供通用符号计算、自动证明、通用碰撞避让或任意曲面展开。支持一种数学表征，不等于覆盖所有题目；新题仍需逐题设计与验收。
