...
const files = ['edge-label.mjs', 'right-triangle.mjs'];
...
const template = await readFile(new URL('triangle-preview.template.html', directory), 'utf8');
const styles = await readFile(new URL('math-style.css', directory), 'utf8');
await writeFile(new URL('triangle-preview.html', directory), template.replace('/* SHARED_STYLES */', styles).replace('/* COMPONENT_SOURCES */', sources));
