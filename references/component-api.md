...
[覆盖范围](component-coverage.md) · [离线预览](../assets/math/library-preview.html)
...
- 字体、标签间距与任意组合参数的避让不是自动保证。已验证范围见 本包交付说明中的验证范围。

必要模型检查：在 skill 目录运行 `node --test assets/math/tests/library.test.mjs`。离线预览随包提供；模块更新后需在开发项目重建预览再同步交付副本。
